...
BEGIN
    ...
    UPDATE auctions_matches SET price = price - (price * decay_factor) WHERE ack_status IS NULL;;
    UPDATE dkp_holds SET amount = amount - (amount * decay_factor) WHERE id IN (SELECT bid_hold FROM public.auctions_matches WHERE bid_hold IS NOT NULL);;
